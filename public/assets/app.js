...
const endpoints={sandbox:{oauth:'https://api-satusehat-stg.dto.kemkes.go.id/oauth2/v1',fhir:'https://api-satusehat-stg.dto.kemkes.go.id/fhir-r4/v1',kfa:'https://api-satusehat-stg.dto.kemkes.go.id/kfa-v2'},production:{oauth:'https://api-satusehat.kemkes.go.id/oauth2/v1',fhir:'https://api-satusehat.kemkes.go.id/fhir-r4/v1',kfa:'https://api-satusehat.kemkes.go.id/kfa-v2'}};
...
function init(){
  ...
  $('terminologyType').onchange=updateTerminologyMode;$('btnTerminologySearch').onclick=searchTerminology;$('terminologyQuery').addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();searchTerminology()}});$('btnCopyCoding').onclick=copyCoding;$('btnInsertCoding').onclick=insertCoding;
...
function updateEndpoint(){const env=$('environment').value;const ep=endpoints[env];$('oauthUrl').textContent=ep.oauth;$('fhirUrl').textContent=ep.fhir;if($('kfaUrl'))$('kfaUrl').textContent=ep.kfa;$('envBadge').textContent=env.toUpperCase();$('envBadge').classList.toggle('danger',env==='production')}
...
function updateTerminologyMode(){const isKfa=$('terminologyType').value==='kfa';$('kfaProductWrap').hidden=!isKfa;$('terminologyNotice').textContent=isKfa?'KFA menggunakan endpoint KFA v2 resmi SATUSEHAT. Pastikan Access Token sudah tersedia.':'Pencarian memakai dataset lokal di writable/terminology. Jika dataset belum ada, aplikasi memakai starter data untuk pengujian.'}
